...
import { empty, objValues, pluck, spaceBetweenProps } from '@/helpers/functions'
import Textarea from '@/components/ui/form/textarea'
import { useGetAllParticipantsQuery } from '@/redux/api/participant.api'
import {
    useCreateUserTagMutation,
    useGetUserTagsQuery
} from '@/redux/api/user-tag.api'
import CreatableSelect from '@/components/ui/form/creatable-select'
import CreatableMultiSelect from '@/components/ui/form/creatable-multiselect'
import { DesktopDatePicker } from '@mui/x-date-pickers'
import moment from 'moment'
import AntSwitch from '@/components/ui/form/switch/ant-switch'
import {
    useCreateTransactionMutation,
    useUpdateTransactionMutation
} from '@/redux/api/cashbox/transaction.api'
            ...
            createUserTag({ name, type: 'transaction' }).then((result) =>
                requestHandler({
                    ...
                    on200Http: (data) => {
                        setFieldValue(
                            'ids_tags',
                            uniq([...values.ids_tags, parseInt(data.id)])
                        )
                    }
                })
            )
