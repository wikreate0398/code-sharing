...
import ConfirmAmountPopup from '@/components/screens/dashboard/cashbox/transactions/_components/confirm-amount-popup'
...
const TranasctionTable = ({ from, to, type }) => {
    ...
    return (
        <Table>
            ...
            <Tbody>
                {Object.entries(groupBy(data || [], groupedDate)).map(
                    (value) => {
                        const date = value[0]
                        const items = value[1]
                        return (
